...
func (p *ZunProvider) ContainerHadoopNodeFactory(c *zun_container.Container, namespace, name, clusterType string) {
	...
	if c.Status == "Running" {
		if isRunningHadoopNode(c.UUID) {
			...
			storage_ip := c.Environment["hadoopstorage"]   //Add 2020-2-24
			fmt.Println("hadoopstorage",storage_ip)
			//storage_ip := "10.10.87.116"
			tempName := name
			clusterName := tempName[0:strings.LastIndex(tempName[0:strings.LastIndex(tempName, "-")], "-")]
			//clusterName = clusterName[0:strings.LastIndex(clusterName, "-")]
			...
			if numberOfNode == AvailableNumber {
				...
				fmt.Println("storage_ip1",storage_ip)
				p.ConfigAndStartContainerHadoopCluster(namespace, clusterName, c.UUID, storage_ip, clusterType)
			} else if numberOfNode < AvailableNumber {
				p.addnodetocluster(namespace, clusterName, c.UUID, storage_ip)
			}
		}
	}
}
...
//Add Environment ip 2020-2-24
func (p *ZunProvider) ConfigAndStartContainerHadoopCluster(namespace, cName, container_id,storage_ip, clusterType string) {
	...
	fmt.Println("storage_ip2",storage_ip)
	p.WriteHadoopConfigFile(hadoopMasterId, HostsStr, slavesStr,storage_ip)
	for _, v := range hadoopSlaveIdList {
		p.WriteHadoopConfigFile(v, HostsStr, slavesStr,storage_ip)
	}
...
func (p *ZunProvider) StartContainerHadoopCluster(containerId, clusterType string) {
	...
	startHadoopClusterStr := fmt.Sprintf("sh /root/start-cluster.sh")
...
func (p *ZunProvider) addnodetocluster(namespace, cName, containerId ,storage_ip string) {
	// Add numberOfNode
	...
	startDatanodeCommand := zun_container.ExcuteOpts{
		Command:     fmt.Sprint("sh /root/add_node.sh"),
		Run:         true,
		Interacitve: false,
	}
...
func (p *ZunProvider) WriteHadoopConfigFile(containerId, HostsStr, slavesStr,storage_ip string) {
	...
	HostsStr = "\"" + HostsStr + "\""
	slavesStr = "\"" + slavesStr + "\""
	fmt.Println(HostsStr,slavesStr,storage_ip)
	hosts_slave := fmt.Sprintf(	"sh /root/modify.sh %s %s %s", HostsStr, slavesStr, storage_ip)
